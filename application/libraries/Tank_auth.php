...
class Tank_auth
{
	...
	function admin_is_logged_in() {
		...
		if($this->user_is_admin() && $this->is_logged_in()) {
	...
	/**
	 * check if user is admin/logged in and redirect away from admin page if not
	 *
	 * @return void
	 */
	
	function user_not_admin_redirect($url='admin/auth/login') {
	 ...
	 * @return	bool
	 */
	function is_logged_in_activated($activated = TRUE)
	{
	 ...
	 * @return	array
	 */
	function create_user($username, $email, $password, $email_activation, $first_name='', $last_name='', $role='0')
	{
	...
	function update_user($user) {
		...
		// check if username is set and is not already used
		if($user['username'] != $user['cur_username']) {
			if(!$this->ci->users_model->is_username_available($user['username'])) {
				$this->error = array('username' => 'auth_username_in_use');
				$success = FALSE;
			} else {
		...
		// check if email is set and is not already used
		if($user['email'] != $user['cur_email']) {
			if (!$this->ci->users_model->is_email_available($user['email'])) {
				$this->error = array('email' => 'auth_email_in_use');
				$success = FALSE;
			} else {
		...
		if(isset($user['first_name'])) { $data['first_name'] = $user['first_name']; }	
		if(isset($user['last_name'])) { $data['last_name'] = $user['last_name']; }	
		if(isset($user['banned'])) { $data['banned'] = $user['banned']; }
		if(isset($user['ban_reason'])) { $data['ban_reason'] = $user['ban_reason']; } else { $data['ban_reason'] = NULL; }
		if(isset($user['role'])) { $data['role'] = $user['role']; }
		if(isset($user['password'])) {
			// Hash password using phpass
			$hasher = new PasswordHash(
					$this->ci->config->item('phpass_hash_strength', 'tank_auth'),
					$this->ci->config->item('phpass_hash_portable', 'tank_auth'));
			$data['password'] = $hasher->HashPassword($user['password']);
		}	

		// update user data through model
		if($success) {
			return $this->ci->users_model->update_user($data, $user['user_id']);
		}
		return FALSE;
	...
	{
		if (!$this->is_logged_in_activated() AND !$this->is_logged_in_activated(FALSE)) {			// not logged in (as any user)
